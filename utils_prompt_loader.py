from utils_config_handler import prompts_conf
...
def load_rag_prompts():
    ...
    try:
        ...
        logger.error(f"[load_rag_prompts]RAG出错{str(e)}")
        raise e
...
def load_report_prompts():
    try:
        ...
        logger.error(f'[load_prompt_prompts]在yaml配置项中没有report_prompt_path配置项')
        raise e
    try:
        return open(report_prompt_path,'r',encoding='utf-8').read()
    except Exception as e:
        logger.error(f"[report_prompt_path]解析报告生成出错{str(e)}")
        raise e
